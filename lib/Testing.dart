...
class _HomePageState extends State<HomePage> {
  ...
  void handleMyLink(Uri url){
    ...
    print("gvcdbhjnmc vchdmfj");
    print("The Token that i'm interesed in is ${seperatedLink[1]}");
    Get.to((){
      showDialog(context: context, builder: (context){
        return AlertDialog(
          content: Text(seperatedLink[1]),
        );
      });
    });
  }

  buildDynamicLinks(String title,String docId) async {
    String url = "https://evecalendar.page.link";
                                                          ...
                                                          child: Container(
                                                              ...
                                                              Row(
                                                                ...
                                                                children: [
                                                                  ...
                                                                  IconButton(
                                                                      onPressed: () async {
                                                                        print(buildDynamicLinks(snapshot.data.docs[index]['title'].toString(), snapshot.data.docs[index].id));
                                                                        buildDynamicLinks(snapshot.data.docs[index]['title'].toString(), snapshot.data.docs[index].id);

                                                                        // var generateLink= await   createShortDynamicLink(
                                                                        //   'https://console.firebase.google.com/project/eve-calendar-bedaa/firestore/data/~2FEvents~2F${user!.uid}~2FDate~2F${snapshot.data.docs[index].id}'
                                                                            // 'https://console.firebase.google.com/project/calender-app-28c65/firestore/data/~2FEvents~2F${user!.uid}~2FDate~2F${snapshot.data.docs[index].id}'
                                                                        // );
                                                                        // print(generateLink);
